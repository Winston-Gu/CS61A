...
def pingpong(n):
    ...
    def helper(index, num, dir):
        ...
        if(((index % 8 == 0) or (num_eights(index) != 0) )):
            return helper(index + 1, num - dir, -dir)
        return helper(index + 1, num + dir, dir)
    return helper(1,1,1)
...
def count_coins(total):
    ...
    True
    """
    def count_use_largest(total, largest):
        if(total == 0 ):
            return 1
        elif(total < 0):
            return 0
            ...
            return count_use_largest(total - largest, largest) + count_use_largest(total, before_largest_coin(largest))
    return count_use_largest(total, 25)
