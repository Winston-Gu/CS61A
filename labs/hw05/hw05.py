...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            if self.funds == 0:
                return 'Here is your ' + self.kind + '.'
            else:
                change = self.funds
                self.funds = 0
                return 'Here is your ' + self.kind + ' and $' + str(change) + ' change.'
    ...
    def restock(self, amount):
        self.stock += amount
        return 'Current ' + self.kind + ' stock: ' + str(self.stock)
...
def store_digits(n):
    ...
    if n // 10 == 0:
        ...
        return Link(highest, store_digits(rest))
...
def is_bst(t):
    ...
    if t.is_leaf() == True:
        ...
        if len(t.branches) == 2:
                return (t.branches[0].label <= t.label and  t.branches[1].label > t.label 
                        and bst_max(t.branches[0]) <= t.label and bst_min(t.branches[1]) > t.label
                        and is_bst(t.branches[0]) and is_bst(t.branches[1]))
        elif(len(t.branches) == 1):
            return is_bst(t.branches[0])
        else:
            return False
def bst_min(t):
    """Returns the minimum label of bst t"""
    if t.is_leaf():
        return t.label
    else:
        if t.branches[0].label < t.label:
            ...
            return t.label
def bst_max(t):
    """Returns the maximum label of bst t"""
    if t.is_leaf():
        return t.label
    else:
        if len(t.branches) == 2:
            if t.branches[1].label > t.label:
...
def path_yielder(t, value):
    ...
    for leaf in t.branches:
        for path in path_yielder(leaf, value):
            yield [t.label] + path
