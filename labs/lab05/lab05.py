...
def closer_city(lat, lon, city_a, city_b):
    ...
    dst_a = distance(city_a, new_city)
    dst_b = distance(city_b, new_city)
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[x // 2] if x % 2 == 0 else deck[x // 2 + len(deck) // 2] for x in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    if(is_leaf(t1) and is_leaf(t2)):
        ...
        return tree(label(t1) + label(t2), 
                    [add_trees(x, y) for x, y in zip(branches(t1), branches(t2))] 
                    + [branches(t1)[len(branches(t2)) + i] if len(t1) > len(t2) else branches(t2)[len(branches(t1)) + i] for i in range(max(len(t1), len(t2)) - min(len(t1), len(t2)))])
