...
def time_per_word(times_per_player, words):
    ...
    """
    # BEGIN PROBLEM 9
    time = [[0 for i in range(len(words))] for j in range(len(times_per_player))]
    for i in range(len(words)):
        for j in range(len(times_per_player)):
            time[j][i] = times_per_player[j][i+1] - times_per_player[j][i]
...
def fastest_words(game):
    ...
    for i in word_indices:
        for j in player_indices:
            if(time(game, j, i) == min(time(game, j, i) for j in player_indices)):
                result[j].append(word_at(game, i))
                break
