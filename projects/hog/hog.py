...
def swine_align(player_score, opponent_score):
    ...
    if(player_score < 0 or opponent_score < 0):
        return False
    for i in range(min(player_score, opponent_score),9,-1):
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0,score1):
        if(who == 0):
            new_score = score0 - last_score
            if(new_score > running_high):
                print(new_score, "point(s)! The most yet for Player 0")
                return announce_highest(who,score0,new_score)
            else:
                return announce_highest(who,score0,running_high)
        if(who == 1):
            new_score = score1 - last_score
            if(new_score > running_high):
                print(new_score, "point(s)! The most yet for Player 1")
                return announce_highest(who,score1,new_score)
            else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_result = 0
    for i in range(1,11):
        function = make_averaged(roll_dice,trials_count)
        result = function(i,dice)
