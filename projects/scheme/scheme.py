...
Date: 2022-02-20 16:08:48
版权信息: 
最后修改者: 高嘉伟(gaojw20@mails.tsinghua.edu.cn)
LastEditTime: 2022-02-20 16:08:48
修改日志: 
备注: 
'''
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 10
        LocalFrame = Frame(self)
        for _ in range(len(formals)):
            LocalFrame.define(formals.first, vals.first)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        if isinstance(expressions.rest.first, int) or isinstance(
                expressions.rest.first, float):
            result = expressions.rest.first
        else:
        ...
        # BEGIN PROBLEM 9
        proc = do_lambda_form(Pair(target.rest, expressions.rest), env)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    proc = LambdaProcedure(formals, expressions.rest, env)
    return proc
    # END PROBLEM 8
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            ...
            if clause.rest is nil:
                ...
                return do_and_form(clause.rest, env)
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        validate_form(current, 2, 2)
        name = current.first
        value = scheme_eval(current.rest.first, env)
    ...
    def evaluate(self):
        if self.expression is not None:
            ...
            if not (value is nil or isinstance(value, Pair)):
                raise SchemeError(
                    "result of forcing a promise should be a pair or nil, but was %s"
                    % value)
            self.value = value
